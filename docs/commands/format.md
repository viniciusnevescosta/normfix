...
Summary: files: 1 | proposed: 1 | written: 1 | fixes: 17 | remaining: 0 | info: 0 | failed: 0
Completed in 0.62 s.
```
...
```sh
normfix --format json format src
```
...
```json
{
  ...
  "summary": { "files": 1, "changed": 1, "written": 1, "fixes": 1, "remaining": 0, "failed": 0 },
